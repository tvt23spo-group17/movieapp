...
app.get('/movies-trending', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch trending movies from TMDb' });
  }
});
...
app.get('/movies-upcoming', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch upcoming movies from TMDb' });
  }
});
  ...
  try {
    ...
    if (mappingResult.rows.length > 0) {
      ...
      if (response.data.results && response.data.results.length > 0) {
          ...
          // Check if the tmdb id exists in the database
          const tmdbCheck = await pool.query(
            'SELECT tmdb_id FROM movie_mappings WHERE tmdb_id = $1',
            [tmdb_id]
          );
          if (tmdbCheck.rows.length > 0) {
            return res.json({ tmdb_id });
          }
